...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    if (auxpow != NULL) {
        ...
        // print to log
        LogPrintf("AidenMiner: AUX proof-of-work found; our hash: %s ; parent hash: %s ; target: %s\n",
               hash.GetHex().c_str(),
    ...
    else
    {
        ...
        // print to log
        LogPrintf("AidenMiner: proof-of-work found; hashPoW: %s ; target: %s\n", hashPoW.GetHex().c_str(), hashTarget.GetHex().c_str());
    }
    ...
    while(true)
    {
        ...
        while(true)
        {
            ...
            while(true)
            {
                ...
                scrypt_N_1_1_256_sp_sse2(BEGIN(pblock->nVersion), BEGIN(thash), scratchpad, 10);
#else
                // Detect SSE2: 32bit x86 Linux or Windows
                scrypt_N_1_1_256_sp(BEGIN(pblock->nVersion), BEGIN(thash), scratchpad, 10);
#endif
#else
                // Generic scrypt
...
void static ScryptMinerOG(CWallet *pwallet)
{
    // Each thread has its own key and counter
    CReserveKey reservekey(pwallet);
    unsigned int nExtraNonce = 0;
...
#if defined(_M_X64) || defined(__x86_64__) || defined(_M_AMD64) || (defined(MAC_OSX) && defined(__i386__))
                // Always SSE2: x86_64 or Intel MacOS X
                scrypt_N_1_1_256_sp_sse2(BEGIN(pblock->nVersion), BEGIN(thash), scratchpad, 6);
#else
                // Detect SSE2: 32bit x86 Linux or Windows
                scrypt_N_1_1_256_sp(BEGIN(pblock->nVersion), BEGIN(thash), scratchpad, 6);
#endif
#else
                // Generic scrypt
                scrypt_N_1_1_256_sp_generic(BEGIN(pblock->nVersion), BEGIN(thash), scratchpad, 6);
#endif
...
{
    LogPrintf("AidenMiner miner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    ...
    {
        LogPrintf("AidenMiner miner terminated\n");
        throw;
    }
}
