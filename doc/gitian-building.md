...
*Setup instructions for a gitian build of Aiden using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Aiden
...
Clone the git repositories for aiden and gitian and then checkout the aiden version that you want to build.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/aiden-org/aiden.git
cd aiden
...
Follow the instructions in [doc/release-process.md](release-process.md) in the aiden repository
...
To build Aiden (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the aiden repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/aiden/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/aiden-org/aiden
...
For example:
```bash
URL=https://github.com/user/aiden.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit aiden=${COMMIT} --url aiden=${URL} ../aiden/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit aiden=${COMMIT} --url aiden=${URL} ../aiden/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit aiden=${COMMIT} --url aiden=${URL} ../aiden/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/aiden-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/aiden-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/aiden-build.assert
```
...
[aiden/gitian.sigs](https://github.com/aiden-org/gitian.sigs.frk/) repository, or if that's not possible create a pull
request. 
